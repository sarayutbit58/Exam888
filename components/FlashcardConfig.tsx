      ...
      <div className="w-full max-w-2xl mx-auto">
        <header className="text-center mb-8">
            <div className="flex justify-center items-center gap-4 mb-4">
                <IconLayers />
                <h1 className="text-4xl sm:text-5xl font-bold text-slate-900 dark:text-white tracking-tight">Flashcard Review</h1>
            </div>
        ...
        <div className="bg-white dark:bg-slate-800/50 border border-gray-200 dark:border-slate-700 p-8 rounded-2xl shadow-2xl space-y-8">
            <div>
                <h2 className="text-xl font-semibold text-slate-900 dark:text-white mb-4">Select Domains</h2>
                    ...
                    {allDomains.map(domain => (
                        <label key={domain} className="flex items-center space-x-3 p-3 bg-gray-50 dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 cursor-pointer hover:bg-gray-100 dark:hover:bg-slate-700/50 transition-colors">
                            <input
                                ...
                                className="h-5 w-5 rounded bg-gray-100 dark:bg-slate-900 border-gray-300 dark:border-slate-600 text-violet-500 focus:ring-violet-500 focus:ring-offset-white dark:focus:ring-offset-slate-800"
                            />
                            <span className="text-slate-700 dark:text-slate-300">{domain.replace(/ \(.+\)/, '')}</span>
                        </label>
                    ))}
                </div>
                <button onClick={toggleSelectAll} className="mt-4 text-sm text-violet-600 dark:text-violet-400 hover:text-violet-500 dark:hover:text-violet-300">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4 border-t border-gray-200 dark:border-slate-700">
                ...
                >
                 ...
                 <button
                    ...
                    className="flex-1 flex items-center justify-center bg-violet-500 hover:bg-violet-400 text-white dark:text-slate-900 font-bold py-3 px-6 rounded-lg transition-transform transform hover:scale-105 active:scale-95 disabled:bg-slate-400 dark:disabled:bg-slate-600 disabled:cursor-not-allowed disabled:scale-100"
                >
                    Start Review
                </button>
            </div>
