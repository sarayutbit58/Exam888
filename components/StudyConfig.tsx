      ...
      <div className="w-full max-w-2xl mx-auto">
        <header className="text-center mb-8">
            <div className="flex justify-center items-center gap-4 mb-4">
                <IconBrain />
                <h1 className="text-4xl sm:text-5xl font-bold text-white tracking-tight">Advanced Study</h1>
            </div>
        ...
        <div className="bg-slate-800/50 border border-slate-700 p-8 rounded-2xl shadow-2xl space-y-8">
            <div>
                <h2 className="text-xl font-semibold text-white mb-4">1. Select Study Mode</h2>
                <div className="grid grid-cols-1 gap-4">
                  {(['all-domains', 'incorrect', 'flagged'] as StudyMode[]).map(mode => (
                    <label key={mode} className={`flex items-center space-x-3 p-3 bg-slate-800 rounded-lg border-2 ${studyMode === mode ? 'border-cyan-500' : 'border-slate-700'} ${ (mode === 'incorrect' && !hasIncorrect) || (mode === 'flagged' && !hasFlagged) ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:bg-slate-700/50'}`}>
                        <input
                            type="radio"
                            name="studyMode"
                            value={mode}
                            ...
                            className="h-5 w-5 bg-slate-900 border-slate-600 text-cyan-500 focus:ring-cyan-500 focus:ring-offset-slate-800 disabled:opacity-50"
                        />
                        <span className="text-slate-300 capitalize">{mode.replace('-', ' ')}</span>
                        {mode === 'incorrect' && !hasIncorrect && <span className="text-xs text-slate-500 ml-auto">(No incorrect answers yet)</span>}
                        {mode === 'flagged' && !hasFlagged && <span className="text-xs text-slate-500 ml-auto">(No flagged questions yet)</span>}
                    </label>
                  ))}
                </div>
            </div>
            ...
            {studyMode === 'all-domains' && (
              <>
                <div>
                    <h2 className="text-xl font-semibold text-white mb-4">2. Select Domains</h2>
                        ...
                        {allDomains.map(domain => (
                            <label key={domain} className="flex items-center space-x-3 p-3 bg-slate-800 rounded-lg border border-slate-700 cursor-pointer hover:bg-slate-700/50 transition-colors">
                                <input
                                    ...
                                    className="h-5 w-5 rounded bg-slate-900 border-slate-600 text-cyan-500 focus:ring-cyan-500 focus:ring-offset-slate-800"
                                />
                                <span className="text-slate-300">{domain}</span>
                            </label>
                        ))}
                    </div>
                    <button onClick={toggleSelectAll} className="mt-4 text-sm text-cyan-400 hover:text-cyan-300">
                        {selectedDomains.length === allDomains.length ? 'Deselect All' : 'Select All'}
                    </button>
                </div>
            
                <div>
                     <h2 className="text-xl font-semibold text-white mb-4">3. Number of Questions</h2>
                     <select
                        value={questionCount}
