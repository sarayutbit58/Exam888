...
const IconCheckCircle = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-6 w-6 text-green-400 mr-3 flex-shrink-0"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>;
        ...
        <div className="min-h-screen bg-slate-900 text-slate-100 p-8 font-sans">
            <div className="max-w-4xl mx-auto">
                <header className="mb-8">
                    <h1 className="text-4xl font-bold text-white mb-2">Unit Test Results</h1>
                    <p className="text-slate-400">Running automated tests for application logic.</p>
                </header>
                ...
                <div className={`mb-8 p-4 rounded-lg border ${summaryColor} flex justify-between items-center`}>
                    ...
                    <div className={`px-4 py-1 rounded-full text-sm font-semibold ${failedCount > 0 ? 'bg-red-400 text-black' : 'bg-green-400 text-black'}`}>
                ...
                <div className="space-y-4">
                    {isRunning && (
                        <div className="flex items-center justify-center p-8 bg-slate-800 rounded-lg">
                            <IconLoader />
                            <span className="text-lg text-slate-300">Executing tests...</span>
                        </div>
                    )}
                    {results.map((result, index) => (
                        <div key={index} className="bg-slate-800 p-4 rounded-lg border border-slate-700">
                            <div className="flex items-start mb-2">
                                ...
                                <h2 className={`font-semibold ${result.passed ? 'text-slate-100' : 'text-red-300'}`}>{result.name}</h2>
                            </div>
                            {!result.passed && (
                                <div className="bg-slate-900 p-3 rounded-md mt-2 ml-9">
